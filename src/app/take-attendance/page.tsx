...
export default function TakeAttendance() {
  ...
  const [date, setDate] = useState<Date | null>(new Date()); // Fixed duplicate state

  useEffect(() => {
    const handleOutsideClick = (event: MouseEvent) => {
      if (sidebarOpen && event.target instanceof Element && !event.target.closest(".sidebar")) {
    ...
    <div className={`flex h-screen ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-black'}`}>
      ...
      <div className={`fixed inset-y-0 left-0 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform md:relative md:translate-x-0 w-64 p-5 space-y-6 ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-md sidebar z-40`}>
        ...
        <nav className="mt-6 space-y-4">
          <Link href="/" className="block p-2 rounded-md hover:bg-gray-300">Home</Link>
          <Link href="/attendance-records" className="block p-2 rounded-md hover:bg-gray-300">Attendance Record</Link>
          <Link href="#" className="block p-2 rounded-md hover:bg-gray-300">Library Record</Link>
          <Link href="#" className="block p-2 rounded-md hover:bg-gray-300">Take Attendance</Link>
          <Link href="/profile-page" className="block p-2 rounded-md hover:bg-gray-300">My Profile</Link>
          <Link href="/add-posts" className="block p-2 rounded-md hover:bg-gray-300">Add Posts</Link>
        </nav>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <nav className={`md:hidden fixed bottom-0 left-0 right-0 p-4 border-t flex justify-around items-center ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} z-30`}>
          <FiPlusCircle className="w-6 h-6 cursor-pointer hover:opacity-80" onClick={() => router.push('/add-posts')} />
          <FiHome className="w-6 h-6 cursor-pointer hover:opacity-80" onClick={() => router.push('/')} />
          <FiMessageCircle className="w-6 h-6 cursor-pointer hover:opacity-80" onClick={() => router.push('/messages')} />
        </nav>
      </div>
    </div>
  );
